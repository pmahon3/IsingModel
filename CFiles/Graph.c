...
void simEpidemic(graph* g, int burnin, int timesteps ){
  ...
  for ( int time = 0 ; time < timesteps ; time++ ){
	...
	for ( int i = 0 ; i < g->infvec[node] ; i++ ){
	  if ( compareDouble( g->phi, genrand()) ){
	...
	if ( compareDouble( g->eta, genrand() ) ){
  ...
  g->avginfected = (double) cuminf / (double) timesteps / (double) g->size;
}
